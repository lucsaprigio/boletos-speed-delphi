...
procedure Tfrm_boletos_speed.dbGridDadosDrawColumnCell(Sender: TObject;
  const [Ref] Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
var
  Grid: TDBGrid;
...
begin
  ...
  if (Grid.DataSource = nil) or (Grid.DataSource.DataSet = nil) or (not Grid.DataSource.DataSet.Active) then
  begin
    Grid.DefaultDrawColumnCell(Rect, DataCol, Column, State);
    Exit;
  end;

  Dias := Grid.DataSource.DataSet.FieldByName('SP_DIAS').AsInteger;

  if (Dias > 5) and not (gdSelected in State) then
  begin
    Grid.Canvas.Brush.Color := $00CBC0FF; // Cor em Hex (RGB invertido) ou clFuchsia
